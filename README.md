...
older `--gpus all` flag). Mount this repo's directory into the container, e.g.
`-v /path/to/purkinje_cell_segmentation:/workspace -w /workspace`.
...
docker run --rm \
  ...
  -v /path/to/purkinje_cell_segmentation:/workspace \
  -v ~/.cellpose:/root/.cellpose \
  -w /workspace \
  community.wave.seqera.io/library/python_pip_cellpose:fdf7a8c3a305a26e \
  bash -c "pip install --quiet matplotlib && \
    python scripts/segment_image.py cpsam /workspace/your_image.tif --output-dir /workspace/results"
```
...
docker run --rm \
  ...
  -w /workspace \
  --entrypoint python3 \
  sopa-cellpose-gpu:blackwell \
  scripts/sopa_write_zarr.py \
    --image /workspace/your_image.tif \
    --output /workspace/results/slide.zarr \
    --overwrite
...
docker run --rm \
  ...
  --entrypoint python3 \
  sopa-cellpose-gpu:blackwell \
  scripts/sopa_segment_he.py \
    --zarr /workspace/results/slide.zarr \
    --output /workspace/results/slide_masks.tif \
...
`--cellpose-model cpsam` with `--pretrained-model /workspace/models/<model_name>` (see
"Model management" below). `--backend none` runs tiles sequentially, which is the safest
option on GPU; `--backend dask` or `--backend threadpool` may be faster but can hit CUDA
context errors across workers.
